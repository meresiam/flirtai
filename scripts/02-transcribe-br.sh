#!/bin/bash
# Transcreve MP3s de um mind BR via mlx-whisper (local, Apple Silicon).
# IDÊNTICO ao 02-transcribe.sh mas com language='pt'.
# Mantido separado pra não mexer no script principal usado pelos gringos.
# Uso: ./02-transcribe-br.sh <mind_id>
...
echo "=== Transcribe BR :: $MIND :: $(date) ===" | tee "$LOG_FILE"
echo "Audio dir: $AUDIO_DIR" | tee -a "$LOG_FILE"
echo "Output:    $OUTPUT_DIR" | tee -a "$LOG_FILE"
echo "Model:     $MODEL" | tee -a "$LOG_FILE"
echo "Language:  pt" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
...
echo "" | tee -a "$LOG_FILE"
echo "=== Transcribe BR finished :: $MIND :: $(date) ===" | tee -a "$LOG_FILE"
